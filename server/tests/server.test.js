...
describe('GET /todos/:id', () => {
	...
	it('should return 404 if invalid ID number', done => {
		request(app)
			//id=123 is invalid
			.get(`/todos/123`)
			.set('x-auth', users[0].tokens[0].token)
			//${todos[1]._id.toHexString()}
			.expect(404)
			.end(done);
	});
});
	...
	it('should update the todo', done => {
		...
		request(app)
			...
			.expect(res => {
				...
				// OLD version expect(res.body.todo.completedAt).toBeA('number');
...
describe('GET /users/me', () => {
	...
	it('should return 401 if not authenticated', done => {
		request(app)
			...
			.expect(401)
			.expect(res => {
				//use .toEqual when comparing two objects
...
describe('POST /users', () => {
	it('should create a user', done => {
		...
		request(app)
			...
			.expect(200)
			.expect(res => {
				// user bracket, not dot notation because x-auth has a hyphen in it (invalid in dot notation)
